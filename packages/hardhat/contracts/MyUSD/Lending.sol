...
contract Lending is Ownable {
    ...
    function withdrawCollateral(uint256 amount) public {
        ...
        emit CollateralWithdrawn(msg.sender, amount, i_coinDEX.currentPrice()); // Emit event for collateral withdrawal
    }
    ...
    function calculateCollateralValue(address user) public view returns (uint256) {
        ...
        return (collateralAmount * i_coinDEX.currentPrice()) / 1e18; // Calculate collateral value in CORN
    }
    ...
    function _calculatePositionRatio(address user) internal view returns (uint256) {
        uint256 borrowedAmount = s_userBorrowed[user]; // Get user's borrowed amount
    ...
    function liquidate(address user) public {
        ...
        emit Liquidation(user, msg.sender, amountForLiquidator, userDebt, i_coinDEX.currentPrice());
    }
    ...
    function getMaxBorrowAmount(uint256 ethCollateralAmount) public view returns (uint256) {
        ...
        uint256 collateralValue = (ethCollateralAmount * i_coinDEX.currentPrice()) / 1e18;
    ...
    function getMaxWithdrawableCollateral(address user) public view returns (uint256) {
        ...
        uint256 ethValueOfPotentialBorrowingAmount = (potentialBorrowingAmount * 1e18) / i_coinDEX.currentPrice();
    ...
    function flashLoan(IFlashLoanRecipient _recipient, uint256 _amount, address _extraParam) public {
        ...
        i_myUSD.mintTo(address(_recipient), _amount);
